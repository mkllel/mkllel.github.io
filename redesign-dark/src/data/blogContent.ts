...
import { CHILDCARE_PROJECT_ID } from './portfolioContent';

interface BlogEnhancement {
  priority: number;
  featured?: boolean;
...
const enhancements: Record<string, BlogEnhancement> = {
  '스크래핑이라고 부르기엔 좀 빡셌던 Selenium 자동화 프로젝트': {
    priority: 1,
    featured: true,
    relatedProjectId: CHILDCARE_PROJECT_ID,
    relatedProjectTitle: '어린이집 B2B 행정 자동화 플랫폼',
    sequence: ['API 요청 접수', '인증 세션 생성', '기관별 단계 실행', '상태·오류 기록', '파일 저장·응답'],
    appendix: `
## 운영 관점에서 정리한 설계 기준
...
`,
  ...
  'GitHub Pages에서 React Router 새로고침 404 문제 해결하기': {
    priority: 2,
    featured: true,
    appendix: `
## 404 fallback의 실제 동작
...
`,
  ...
  '1년 만의 포트폴리오 리뉴얼: React + TypeScript + Firebase 도입기': {
    priority: 4,
    appendix: `
## 배포 구조에서 배운 점
...
`,
  ...
  '처음 써본 TypeScript: 시행착오부터 느낀 점까지': {
    priority: 5,
    appendix: `
## 실제로 막았던 타입 문제
...
export const getBlogPresentation = (post: BlogPost): BlogPresentation => {
  const enhancement = enhancements[post.title];
  const cleanedContent = post.title === '내가 백엔드 서버로 FastAPI를 고집하는 이유'
    ? post.content.replace(/["“”']?패스트에이피아이\s*야호[^\n]*\n?/u, '')
    : post.content;

  return {
    priority: enhancement?.priority ?? 99,
    featured: enhancement?.featured ?? false,
    content: `${cleanedContent}${enhancement?.appendix ?? ''}`,
    tags: normalizeTags(post.tags),
    sequence: enhancement?.sequence,
    ...(enhancement?.relatedProjectId && enhancement.relatedProjectTitle
      ? {
          relatedProject: {
            id: enhancement.relatedProjectId,
            title: enhancement.relatedProjectTitle,
          },
        }
      : {}),
  };
